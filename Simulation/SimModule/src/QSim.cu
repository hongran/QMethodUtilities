...
*/
__global__ void make_randfill( curandState *state, float *pulseTemplate, float *pedestalTemplate, float *energySumArray, float *flushHitArray,float *flushQTruthArray,  float *flushQArray, int *SimulationIntParameters, float * SimulationParameters){
...
*/
       ...
       atomicAdd( &(energySumArray[ itick ]), Elab); // diagnostic for energy time distributions
...
//      printf( "TemplateZero = %d tick= %f  itick= %d  rtick =%f TickEdgeIndex = %d TimeBinFractions[0] = %f \n",TemplateZero,tick,itick,rtick,TickEdgeIndex,TimeBinFractions[0]);
...
//	if(TimeBinFractions[tempIdx] < -0.3)printf("tempindex:%d ; left idx: %d ; right index: %d ; fraction %f\n",tempIdx,leftIdx,rightIdx,TimeBinFractions[tempIdx]);
      }
      ...
      //Calculating the gaussian integral for each x or y segments
       ...
       for (int iy = 0; iy < nyseg; iy++) {
           SegmentY[iy] = 0.5*(-erfcf((iy+1.0-ycoord)/(sqrt(2.)*ysig))+erfcf((iy-ycoord)/(sqrt(2.)*ysig)));
       }
       for (int ix = 0; ix < nxseg; ix++) {
...
//	     if (ADCfrac<-100.0)printf("%f %f\n",fsegment,TimeBinFractions[k]);

	     //printf("xtal %i, ADCfrac %f\n", ix+iy*nxseg, ADCfrac);
	     atomicAdd( &(flushQTruthArray[ iflush*nsegs*fill_buffer_max_length + xysegmentoffset + kk ]), ADCfrac/TIMEDECIMATION); // divide by time decimation to account for data processing calculates the time-decimated average not sum
	     atomicAdd( &(flushQArray[ iflush*nsegs*fill_buffer_max_length + xysegmentoffset + kk ]), ADCfrac/TIMEDECIMATION); // Keep this array only to this fill and use it for analysis, add pedestal later
	   }
	 ...
	 for (int k=35; k<fill_buffer_max_length; k++) {
	   atomicAdd( &(flushQArray[ iflush*nsegs*fill_buffer_max_length + xysegmentoffset + k ]), pedestalTemplate[k]); 
	 }
//	 printf("\n ");
...
{
  QSim::QSim(const std::map<std::string,int>& tIntParameters,const std::map<std::string,float> & tFloatParameters, const std::map<std::string,std::string> & tStringParameters, long long int tSeed)
  {
    //Import Parameters
    ...
    {
      int ret = IntegratePulseTemplate(StringParameters["Pulse Template Path"],25,IntParameters["TemplateSize"],IntParameters["TemplateZero"]);
    }else{
      IntegratedPulseTemplate = std::vector<float>(IntParameters["TemplateSize"]);
      for (int i=IntParameters["TemplateZero"];i<IntParameters["TemplateSize"];i++)
      {
    ...
    {
      PedestalTemplate = std::vector<float>(IntParameters["FillBufferMaxLength"],0.0);
      int ret = LoadPedestalTemplate(StringParameters["Pedestal Template Path"],0);
    }else{
      PedestalTemplate = std::vector<float>(IntParameters["FillBufferMaxLength"],0.0);
    }

    //Allocate Derive memory for templates
    cudaMalloc( (void **)&d_IntegratedPulseTemplate, IntParameters["TemplateSize"]*sizeof(float));
    cudaMalloc( (void **)&d_PedestalTemplate, IntParameters["FillBufferMaxLength"]*sizeof(float));
    
    cudaMemcpy( d_IntegratedPulseTemplate,&IntegratedPulseTemplate[0], IntParameters["TemplateSize"]*sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy( d_PedestalTemplate,&PedestalTemplate[0], IntParameters["FillBufferMaxLength"]*sizeof(float), cudaMemcpyHostToDevice);
    ...
    ArraySizes["EnergySumArray"] = IntParameters["FillBufferMaxLength"]*sizeof(float); 
    ArraySizes["FlushHitArray"] = nFlushesPerBatch*IntParameters["FillBufferMaxLength"]*sizeof(float); 
    ArraySizes["FlushQTruthArray"] = nFlushesPerBatch*nsegs*IntParameters["FillBufferMaxLength"]*sizeof(float); 
    ArraySizes["FlushQArray"] = nFlushesPerBatch*nsegs*IntParameters["FillBufferMaxLength"]*sizeof(float); 
  ...
  int QSim::Simulate(int NFlushes)
  {
    ...
    for (int i=0;i<NSim;i++)
    {
      ...
      {
	cudaMemset( DeviceArrays["FlushHitArray"], 0.0, ArraySizes["FlushHitArray"]);
	cudaMemset( DeviceArrays["FlushQTruthArray"], 0.0, ArraySizes["FlushQTruthArray"]);
	cudaMemset( DeviceArrays["FlushQArray"], 0.0, ArraySizes["FlushQArray"]);
      }

      make_randfill<<<nblocks,IntParameters["NThreadsPerBlock"]>>>( d_state,d_IntegratedPulseTemplate,d_PedestalTemplate, DeviceArrays["EnergySumArray"], DeviceArrays["FlushHitArray"], DeviceArrays["FlushQTruthArray"], DeviceArrays["FlushQArray"], d_SimulationIntParameters, d_SimulationParameters);
...
//      std::cout<< n << " "<<Name<<" "<<Size<<std::endl;
  ...
  int QSim::Output(int RunNumber)
  {
    ...
    TH1 * hLastFlushQ = new TH1D("LastFlushQHist","LastFlushQHist",N,0,N*0.075);
    ...
    TH1 * hLastFlushQTruth = new TH1D("LastFlushQTruthHist","LastFlushQTruthHist",N,0,N*0.075);
    for (unsigned int i=0;i<N;i++)
    {
      if (i>3100)break;
    ...
    TFile* FileOut = new TFile(Form("SimulatorOut_%04d.root",RunNumber),"recreate");
    hLastFlushQ->Write();
  ...
  int QSim::IntegratePulseTemplate(std::string TemplatePath,int CrystalId,int TemplateSize,int TemplateZero)
  {
